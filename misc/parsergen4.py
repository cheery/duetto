...
parser.add_argument('--lr0',
    dest='flavor', default='lr', action='store_const', const='lr0',
    help='Generate LR0 tables instead of LR')
...
def st_0(tok, ch):
    if ch.isdigit():
        ...
        print("error token: {} at {}".format(ch.encode('utf-8'), tok.line))
        advance('error', ch,
...
def print_item(prefix, (rule, index), fd=sys.stdout):
    ...
    rhs = map((lambda x: x.encode('utf-8')), rhs)
...
for mapping in lr_mapping:
    ...
    if len(had_conflicts) > 0:
        ...
        for sym in had_conflicts:
            print_itemset('  {}'.format(sym.encode('utf-8')), lr_conflicts[(mapping,sym)],
                sys.stderr)
            if tab[sym][0] != 1:
                sys.stderr.write("  {}: shift to {}\n".format(sym.encode('utf-8'),
...
if parser_flavor == 'lr':
    ...
    if python_output is not None:
        with open(python_output, 'w') as fd:
            ...
            fd.write("\nlexemes = ")
            pw.pprint(list(lexemes))
            def before_dot((rule, index)):
                lhs, rhs = grammar[rule]
                return rhs[:index]
            priors = []
            for index in range(len(seed_itemsets)):
                priors.append(max([before_dot(r) for r in seed_itemsets[index]], key=len))
            fd.write("\npriors = ")
            pw.pprint(list(priors))


elif parser_flavor == 'lr0':
    with open(python_output, 'w') as fd:
        pw = pprint.PrettyPrinter(indent=2, stream=fd)
        lr0_symtab = list(sorted(lexemes))
        for lhs,rhs in grammar:
            if lhs not in lr0_symtab and lhs is not None:
                lr0_symtab.append(lhs)
        fd.write("symtab = {!r}\n".format(lr0_symtab))
        lr0_rtab = []
        for lhs, rhs in grammar:
            rtab_entry = []
            for sym in reversed(rhs):
                rtab_entry.append((sym == "TEXT" or (sym not in lexemes)))
            if lhs is None:
                lr0_rtab.append( (len(lr0_symtab), rtab_entry) )
            else:
                lr0_rtab.append( (lr0_symtab.index(lhs), rtab_entry) )
        fd.write("rtab = ")
        pw.pprint(lr0_rtab)
        lr0_map = {}
        for i in range(len(shifts)):
            assert len(reductions[i]) <= 1, (
                "itemset {} has LR0 reduce/reduce".format(i))
            assert len(shifts[i]) == 0 or len(reductions[i]) == 0, (
                "itemset {} has LR0 shift/reduce".format(i))
            if len(shifts[i]) > 0:
                lr0_map[i] = len(lr0_map)
        lr0_state = []
        for sh in shifts:
            tab = [0]*len(lr0_symtab)
            if len(sh) == 0:
                continue
            for sym, to in sh:
                if to in lr0_map:
                    tab[lr0_symtab.index(sym)] = lr0_map[to]
                else:
                    tab[lr0_symtab.index(sym)] = (-1
                        -list(reductions[to])[0][0])
            lr0_state.append(tab)
        fd.write("state = ")
        pw.pprint(lr0_state)
